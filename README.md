...
└── js/
    ...
    ├── awaitingInfo.js     ← דף 3: אבידות שמחכות למידע (ללא מספור)
    ├── closedItems.js      ← היסטוריית אבידות שהוחזרו או נמחקו
    ├── managerActions.js   ← משיכת/מחיקת אבידות (אחמ"ש)
...
### 2. דפי אבידות (3 קטגוריות + היסטוריה)
- חיפוש גלובלי וסינון לפי תאריך.
- המספור משותף ל`lostItems` ול־`pendingPickup`, כך שכל אבידה מקבלת מספר ייחודי ברצף אחד.
...
- משתמש במספור המשותף עם דף האבידות.

### 5. דף 3 — אבידות שמחכות למידע
- הרשומות נוצרות ללא מספר אבידה. מספר חדש מוקצה רק בעת העברה לדף האבידות או לדף ממתינות לאיסוף.
- כפתור "העבר אבידה" שמוביל לדף 1 או דף 2 ופותח שם את מודל ההוספה כשהוא ממולא מראש; בעת השמירה, הרשומה המקורית נמחקת.

### 6. אבידות שהוחזרו / נמחקו
- כל החזרה או מחיקה מכל שלושת סוגי האבידות מועברת לאוסף `closedItems` ומוצגת בעמוד היסטוריה אחד.
- מחיקה מאפשרת לשמור פירוט לסיבת המחיקה, גם במחיקה מרוכזת דרך פעולות אחמ"ש.
- לחיצה על רשומה מציגה את כל פרטי האבידה והתמונה; באבידה שהוחזרה ניתן לפתוח גם את פרטי ההחזרה והחתימה.
- העמוד מציג גם רשומות ישנות שסומנו כמוחזרות לפני הוספת ארכיון ההיסטוריה.
...
/closedItems/{sourceCollection__sourceId}: { ...snapshot, sourceCollection, sourceId,
                                             terminalStatus: "returned"|"deleted",
                                             deletionReason, returnDetails, closedAt, closedByName }
/counters/itemNumber: { value: number } // רצף משותף לאבידות ולממתינות לאיסוף
```
